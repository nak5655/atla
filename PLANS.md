...
## 計画
### アクティブタスク (2026-05-02): `impl A for B` の `A` を .NET インターフェイス限定へ変更

#### ミッション
- `impl A for B` の `for` 句（A）に指定可能な型を .NET インターフェイスのみに制限する。
- `A` が .NET クラス（sealed/abstract 含む）や interface 以外の場合は決定的診断を返す。
- 既存の phase 境界と診断順序を維持する。

#### 実行ステップ
1. `Resolve` の `impl ... for ...` 解決ロジックを見直し、`for` 句が import 済み .NET 型であることを検証する。
2. `for` 句の .NET 型種別判定を追加し、`IsInterface = true` のみ成功させる。
3. .NET クラスを指定した場合の専用エラー診断を追加し、span を `impl` 宣言位置へ関連付ける。
4. 既存の `impl ... for ...` テストを更新し、成功系（interface）/失敗系（class）/重複判定を回帰固定する。
5. `dotnet test` を実行し、意味解析と下流フェーズの非退行を確認する。
