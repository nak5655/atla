...
## 計画
### アクティブタスク (2026-04-27): VS Code 入力時補完の改善（`.` トリガー廃止）

#### ミッション
- VS Code 拡張で、入力中に補完候補を自動表示できるようにする。
- `.` 入力を補完トリガーから除外し、既存のドット入力依存を廃止する。
- 補完の絞り込みをカーソル位置ベースで安定化し、既存 IntelliSense（hover/definition）を非退行で維持する。
