...
- [ ] フェーズ2: 依存解決・参照側（LSP/CLI）での manifest 探索名を `atla.yaml` に統一する。
- [ ] フェーズ3: テストを `atla.yaml` 前提へ更新する（TOML回帰テストは作成しない）。
- [ ] フェーズ4: ドキュメントを YAML 前提記述へ全面置換する（廃止告知は記載しない）。
- [ ] フェーズ5: フルテストスイート実行で回帰なしを確認する。
