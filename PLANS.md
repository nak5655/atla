...
### アクティブタスク (2026-05-02): `impl A for B` の A を .NET インターフェイスに限定

#### ミッション
- `impl A for B` / `impl A for B by b` における `A` が .NET 型の場合、クラスを拒否してインターフェイスのみ許可する。
- 将来の Atla インターフェイス対応に備え、現状の制約を明示診断で固定化する。

#### 実行ステップ
1. Resolver の `impl ... for ...` 基底型解決を更新し、.NET クラスを拒否・.NET インターフェイスのみ受理する。
2. Semantics テストを更新し、`impl DateTime for ...` の失敗系と `impl ICollection for ... by ...` の成功系を追加/置換する。
3. 対象テストを実行して回帰確認する。
