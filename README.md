...
- 単項マイナスをサポートします（例: `-1`, `-1.0`, `-(x + 1)`）。
- ビルトイン算術演算子 `+ - * / ==` は `Int` と `Float` の両方をサポートします（`%` は `Int` のみ）。
- `Std.Prelude`（`Std/Prelude.atla`）はコンパイラへ同梱され、コンパイル対象へ常に含まれます。各モジュールには `import Std'Prelude` が暗黙的に付与されます（`Std.Prelude` 自身は除く）。
