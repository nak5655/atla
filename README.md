...
Atla は、F# 実装でありながら言語デザインとしては次の3点を強く意識しています。

### 1. Concatenative programming

値を前に置き、処理を後ろへ連結していく書き方ができます。  
関数名の後ろにピリオドを置くことで関数呼び出しになります。  
処理はスタックベースではなく、コンパイル時に関数呼び出しの引数は確定します。
...
- プリミティブ型名は `String` / `Int` / `Bool` / `Float` / `Double` / `Unit` のように **大文字始まり**を使用してください。
- 浮動小数点は単精度が `Float`（.NET `float32`）、倍精度が `Double`（.NET `float`）です。リテラルは接尾辞 `f` 付き（例: `1.0f`）が単精度、接尾辞なし（例: `1.0`）が倍精度になります。
...
- 単項マイナスをサポートします（例: `-1`, `-1.0`, `-1.0f`, `-(x + 1)`）。
- ビルトイン算術演算子 `+ - * / ==` は `Int` / `Float` / `Double` をサポートします（`%` は `Int` のみ）。
