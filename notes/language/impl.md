...
.NET クラスを基底クラスとして継承する場合は `struct T: DotNetClass` 構文を使用する。
...
- `struct T: X` で X が .NET クラスの場合、T は X を継承する。
- `override` キーワードは `struct T: X` で native base が定義されている型の `impl T` ブロック内でのみ有効。
- `base` キーワードはインスタンスメソッド内でのみ使用可能。
- interface の継承は不可（`X.IsInterface` チェック）。
- sealed クラスの継承は不可（`X.IsSealed` チェック）。
- .NET class でなければならない（`X.IsClass` チェック）。
