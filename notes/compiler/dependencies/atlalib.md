...
- union バリアント: 独立した型として `type:<module-name>:<variant-type-name>` で表現する
  （バリアント型名は `Color'Rgb` のように `'` 連結した修飾名。専用の case ID は持たない）。

生成規則:

- 宣言名はソース上の公開名をそのまま使う。
- compiler 内部の `SymbolId` や連番を埋め込まない。
- JSON 出力順は `module-name`, `type-name`, `member-name` を UTF-8 byte-wise lexicographic order で比較して決定する。
- hidden field のような import 復元専用要素を公開する場合は、`field:<module-name>:<type-name>:__hidden__<field-name>` のように **フィールド名セグメント先頭** に `__hidden__` 接頭辞を付与し、通常公開フィールドと衝突しないようにする。
- v1 で予約する内部接頭辞は **フィールド名セグメント先頭の** `__hidden__` のみとし、`__internal__` や `__private__` など他の接頭辞を暗黙予約してはならない。
- `my__hidden__field` のように先頭以外に `__hidden__` を含む通常名は予約扱いしない。
...
union は data と同列の曖昧な表現にしてはならない。union は **ルート型 + バリアント型群**
の継承階層として表現する（タグ整数による表現は用いない。詳細は `notes/language/union.md` を参照）。

- ルート型は `kind = union` を持ち、abstract である。
- 各バリアントは独立した型 export として表現し、`baseType` にルート型（ネスト union では
  親 union）を持つ。バリアント型名は `Color'Rgb` のように `'` 連結した修飾名とする。
- バリアント自身のフィールドは当該バリアント型の `fields` に、共有フィールドはルート型の
  `fields` に置く（import 側は `baseType` 連鎖で継承フィールドを合成する）。
- `extendable` union は本体外で追加されたバリアント型も同じ規則で export する。

> 注意（実装状況）: 現状の `Atla.Build` は union 型を `.atlalib` へ export していない
> （型 export は `data` / `role` のみ）。本節は要求仕様であり、`.atlalib` 経由の union 配布は
> 未実装。`compileModules`（ソース全体を同時コンパイル）経由のクロスモジュール union は動作する。
...
- 行順は path を UTF-8 byte-wise lexicographic order で比較した決定順でなければならない。
...
異なる優先順位層に同名候補がある場合は、**上位層の候補が勝ち**、下位層候補は shadow される。
...
- union バリアント型情報（ルート型 + baseType を持つバリアント型群）
- role 情報
...
4. load 成功、または **対象 asset 自体は load 成功しており** chained `FileNotFound` Warning のみが付随した asset を **有効 runtime asset** とみなす。

- 当該依存について少なくとも 1 つの runtime asset ロード経路が有効である。
- 利用中の import 文が要求する .NET 型またはネイティブメンバー解決に失敗していない。

ここで許容される Warning は、直接対象 asset の破損やハッシュ不一致ではなく、ロード済み asset が参照する **未使用の連鎖依存** の欠損に限る。
