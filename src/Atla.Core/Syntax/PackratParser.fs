...
module Combinators =
    ...
    /// PackratParser.fs 内部でのみ使用する。Parser.fs からは参照しないこと。
    let private Memo (p: PackratParser<'I, 'A>) : PackratParser<'I, 'A> =
    ...
    let Fail (message: string) : PackratParser<'I, 'A> =
        Memo (fun _ pos -> Failure (message, { left = pos; right = pos }))
