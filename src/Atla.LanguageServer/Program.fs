...
let main _ =
    ...
    while true do
        ...
        with ex ->
            // 予期しない例外が発生した場合はログに記録して終了する。
            // これにより VS Code が "Stopping server failed" ではなく正常な切断として扱える。
            try windowLogMessage (sprintf "Unhandled exception: %s" ex.Message) MessageType.Error with _ -> ()
