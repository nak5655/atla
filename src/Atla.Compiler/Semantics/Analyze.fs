...
module Analyze =
    ...
    let declareLocal (symbolTable: SymbolTable) (scope: Scope) (name: string) : SymbolId =
    ...
    let rec analyzeExpr (symbolTable: SymbolTable) (scope: Scope) (expr: Ast.Expr) (expected: TypeId) : Hir.Expr =
        ...
        | :? Ast.Expr.Block as blockExpr ->
            ...
            | _ ->
                ...
                Hir.Expr.Block(stmts, unitExpr, blockScope, expected, blockExpr.span)
        | :? Ast.Expr.Apply as applyExpr ->
            ...
            List.tail (applyExpr.args) |> List.fold (fun acc arg ->
                ...
                Hir.Expr.Apply(acc, argExpr, expected, applyExpr.span)) func
    ...
    and analyzeStmt (symbolTable: SymbolTable) (scope: Scope) (stmt: Ast.Stmt) : Hir.Stmt =
        ...
        | :? Ast.Stmt.Let as letStmt ->
            ...
            // シンボルテーブルに変数を追加する
            let sym = declareLocal symbolTable scope letStmt.name
        ...
        | :? Ast.Stmt.Var as varStmt ->
            ...
            // シンボルテーブルに変数を追加する
            let sym = declareLocal symbolTable scope varStmt.name
    ...
    let analyzeFn (symbolTable: SymbolTable) (scope: Scope) (fnDecl: Ast.Decl.Fn) : Hir.Field =
        ...
        // 返り値の型を名前解決する
        let retType = resolveTypeExpr scope fnDecl.ret
        ...
        // 関数のシンボルを定義する
        let symInfo = SymbolInfo(fnDecl.name, tid, SymbolKind.Local ())
        let sym = symbolTable.Add(symInfo)
        scope.DeclareVar(fnDecl.name, sym)
    ...
    let analyzeModule (symbolTable: SymbolTable, typeSubst: TypeSubst, moduleName: string, moduleAst: Ast.Module) : Hir.Module =
        ...
        for decl in moduleAst.decls do
            ...
            | :? Ast.Decl.Fn as fnDecl ->
                fields.Add(analyzeFn symbolTable moduleScope fnDecl)
